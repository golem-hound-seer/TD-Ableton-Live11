...
class Listeners:
	...
	def add(self, lomExpression, property, returnAddress, uid, extra=''):
		"""
		...
		:param uid: unique id of watcher
		...
		if registryKey in self.registry:
			...
			self.registry[registryKey]['registrants'].add(uid)
			if lomExpression in SPECIALLISTENERS or \
				...
				createListener = True
				self.remove(lomExpression, property, returnAddress, uid, extra)
			...
			if lomExpression in SPECIALLISTENERS:
				...
				try:
					...
					self.codeContext['TDA'].sendError(
							'Error creating listener from OP uid: ' + str(uid),
							self.client)
					raise
				...
				self.registry[registryKey] = {'registrants':{uid},
		...
		self.client.send(OSCMessage(
				'/listener' + registryKey[2] + '/setterIndex',
					[setterIndex, lomExpression, property, returnAddress,
					 uid, extra] if extra is not None else
					[setterIndex, lomExpression, property, returnAddress,
					 uid]))

	def remove(self, lomExpression, property, returnAddress, uid=None, extra=''):
		"""
		...
		:param uid: unique uid of watcher. If None, remove listener.
		:param extra: an extra string with instructions.
				'noinit' = don't send initial value message
		"""
		registryKey = (lomExpression, property, returnAddress, extra)
		...
		if uid is None:
			...
			if uid in registeredIDs:
				registeredIDs.remove(uid)
		# if lomExpression in SPECIALLISTENERS:
		# 	self.debug(lomExpression, registeredIDs)
		if not registeredIDs:
			if lomExpression in SPECIALLISTENERS:
				self.removeSpecial(lomExpression, property, returnAddress, uid,
								   							extra, registryInfo)
			# remove listener
	...
	def addSpecial(self, lomExpression, property, returnAddress, uid, extra=''):
		"""
		Set up whatever is needed for special .
		:param lomExpression: Special uid. Starts with '*'
		:param property: Any relevant info
		:param returnAddress: OSC address to send updates to
		:param uid: unique uid of watcher
		...
		return {'registrants': {uid},
				'callback': callback,
				'setterIndex': setterIndex,
				'liveObject': liveObject}

	def removeSpecial(self, lomExpression, property, returnAddress, uid, extra,
					  										registryInfo):
		"""
	...
	# region clip trigger quantization

	def addClipTriggerQuantization(self, property, returnAddress, extra=''):
		context = dict(self.codeContext)
		context['OSCMessage'] = OSCMessage
		context['self'] = self
		callbackCode = \
		"def callbackClipTriggerQuantization():\n" \
		"	index = int(SONG.clip_trigger_quantization)\n" \
		"	self.client.send(OSCMessage('" + returnAddress + "', [index]))"
		exec(callbackCode, context)
		callback = context['callbackClipTriggerQuantization']
		self.codeContext['SONG'
							].add_clip_trigger_quantization_listener(callback)
		return {'callback': callback }

	def removeClipTriggerQuantization(
						self, property, returnAddress, extra, registryInfo):
		self.codeContext['SONG'].remove_clip_trigger_quantization_listener(
												registryInfo['callback'])
