...
import math, re, socket, select, string, struct, sys, threading, time, types, errno, inspect # TD , array
...
class OSCServer(UDPServer, OSCAddressSpace):
	...
	def handle_timeout(self):
		UDPServer.handle_timeout(self)
		self.empty = True
	
	def handle_request(self):
		self.empty = False
		UDPServer.handle_request(self)
...
#        self.printErr("%s on request from %s: %s" % (e_type.__name__, getUrlStr(client_address), str(e)))
		...
		self.sendError(traceback.format_exc(), client_address)
