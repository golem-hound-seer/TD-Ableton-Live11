	...
	'tracks': {
		'<track name>': { # track info
			...
			'aClips': [
				...
				'name': '<clip name>',
				'index': '<arrangement clip index>',
				'expression': '<LOM expression within parent>',
				'startTime': start time in beats float,
				'endTime': end time in beats float,
				'startMarker': start marker in beats float,
				'endMarker': end marker in beats float,
				'loopStart': loop start in beats float,
				'loopEnd': loop end in beats float,
				'filePath': '<clip file path>' or ''
				'color': RGB int,
				'ptr': LOM Pointer (not persistent across saves) 
				}
